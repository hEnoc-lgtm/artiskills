...
try {
    ...
    $verifExiste = $pdo->prepare("SELECT idAdresse FROM adresse_atelier WHERE id_artisan = :id_artisan");
    $verifExiste->execute(["id_artisan" => $id_artisan]);
    if ($verifExiste->fetch()) {
        http_response_code(409);
        echo json_encode(["success" => false, "message" => "Cet artisan a déjà une adresse d'atelier enregistrée. Utilisez la modification."]);
        exit;
    }
    ...
    echo json_encode(["success" => true, "message" => "Adresse d'atelier créée avec succès.", "data" => ["idAdresse" => (int) $pdo->lastInsertId()]]);
